...
#print(fin[0])"""
new_fin = fin[0: round(len(fin) * 0.9)]  #filtered lgs for features and not in fams and not within 1000km
sel10 = fin[round(len(fin) * 0.9):] #remaining 
dev_add = sel10[:round(len(sel10) * 0.5)] #falf to dev
test_add = sel10[round(len(sel10) * 0.5):] #half to test 
header = [
...
# select 20% from excluded lgs and the rest from rest of dataset 
#print(len(all_ex))
...
final_test=[]
for row in test:
    if row[0] not in dev_lgs:
        final_test.append(row)

with open(DATA_PATH/"test.tsv", "wt", encoding="utf-8", newline="") as f:
    writer = csv.writer(f, delimiter="\t")
    writer.writerow(header)
    writer.writerows(final_test)
