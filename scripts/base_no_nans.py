...
def bool_to_cat(df, gb_columns):
    #input entire df and return it with gb columns turned to categorical values
...
def label_encode_df(df, gb_columns):
    #input entire df with categorical vals, fits encoders to all values seen in selected cols and returns entire df with encoded vals and encoders
    df_enc= df.copy()
...
def transform_w_encoder(df, gb_columns, encoders):
    #take a df, and encode the gb columns with fitted encoders 
    df_enc= df.copy()
    for col in gb_columns:
...
def decode_pred(df_pred, gb_columns, encoders):
    #reverse encodint and return the entire df wwith gb values decoded 
    df_dec= df_pred.copy()
    for col in gb_columns:
        le= encoders[col]
...
#FILL AND ENCODE 
    df_train= fill_strat(strategy, df_train, gb_columns) #fill only train 
    df_train = bool_to_cat(df_train, gb_columns)
    df_dev= bool_to_cat(df_dev, gb_columns)
#Create dummy unk row so the encoders see it (move to function)
    unk_row = pd.DataFrame([["unk"] * len(gb_columns)], columns=gb_columns)
    df_train_aug = pd.concat([df_train[gb_columns], unk_row], ignore_index=True)
#fit encoders on augmented and then drop the unk row 
    df_train_aug, encoders = label_encode_df(df_train_aug, gb_columns)
    df_train[gb_columns] = df_train_aug.iloc[:-1].values
    df_dev= transform_w_encoder(df_dev, gb_columns, encoders)
    ...
    #print(df_dev.head())
    masked_df_dev, masked_positions= mask_dev(df_dev, gb_columns, mask_ratio= 0.3, seed=30)
    masked_df_dev = bool_to_cat(masked_df_dev, gb_columns)
    masked_df_dev= transform_w_encoder(masked_df_dev, gb_columns, encoders)
...
#EVAL
    masked_eval(gb_columns, df_dev[gb_columns], df_pred, masked_positions, RES_PATH/ f"{model}fixed_results{strategy}.txt")
#DECODE
    y_true = decode_pred(df_dev[gb_columns], gb_columns, encoders)
    ...
    y_pred_interp_decoded.to_csv(OUTPUT_PATH/ f"{model}fixed_output{strategy}.csv", sep="\t", index=False)
