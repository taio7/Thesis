...
#print(all_rows[:5])
#print(f"before filter {len(all_rows)}")
...
for l in all_rows:
    ...
    if c > 50:
          ...
          all4.append(new_row)
#print(all4[0])
#print(f"after filter {len(all4)}")                             no lgs with that few features 
# find geo-coordinates for some languages
#print(m)
otherlang_coo = []
for m in all4:
    if m['language_family'] == "Mayan" or m['language_family'] == "Tucanoan" or m['language_family'] == 'Sepik' or m['language_family'] == 'Worrorran' or m['language_family'] == 'Tungusic' or m['language_family'] == "Nilotic":
      cc = (m["language_latitude"], m["language_longitude"])
      otherlang_coo.append(cc)
#print(len(otherlang_coo))
...
for m in all4: 
      ...
      if m['language_family'] != "Mayan" or m['language_family'] != "Tucanoan" or m['language_family'] != 'Sepik' or m['language_family'] != 'Worrorran' or m['language_family'] != 'Tungusic' or m['language_family'] != "Nilotic" and  all(i > 1000 for i in distances): 
         w = m['id'] 
         fa = m['language_family']  
         f = list(m.items())[6:] 
         pre_fin.append([w, fa, f]) 
#print(len(pre_fin)) =2424
...
for m in all4:
    ...
    for coord2 in otherlang_coo:
       dist = geopy.distance.geodesic(coord1, coord2)
       distances.append(dist)
    if m['language_family'] != "Mayan" and m['language_family'] != "Tucanoan" and m['language_family'] != 'Sepik' and m['language_family'] != 'Worrorran' and m['language_family'] != 'Tungusic' and m['language_family'] != "Nilotic" and  all(i > 1000 for i in distances): 
        row= [
            m['id'],
            m["language_name"],
            m["language_latitude"],
...
# save train data
with open(DATA_PATH/"train.tsv", "wt", encoding="utf-8") as f:
    print("id  language_name  language_latitude  language_longitude language_macroarea language_family  features", file=f)
    for g in new_fin:
        print(g, file=f)
...
all_ex = []
for l in all_rows:
    if l['language_family'] == "Mayan" or l['language_family'] == "Tucanoan" or l['language_family'] == 'Sepik' or l['language_family'] == 'Worrorran' or l['language_family'] == 'Tungusic' or l['language_family'] == "Nilotic":
...
# save dev data
with open(DATA_PATH/"dev.tsv", "wt", encoding="utf-8") as f:
    print("id  language_name  language_latitude  language_longitude language_macroarea language_family  features", file=f)
    for g in dev:
        print(g, file=f)
